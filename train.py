...
def read_task_data(task_num):
    test_data = pd.read_excel('./datasets/test_data/task_'+str(task_num)+'/test_data.xlsx')
    x = []
    img_path = []
...
def train():
    ...
    model.fit(x_train=(train_x, train_img), y_train=trian_y, x_validate=(vali_x, vali_img), y_validate=vali_y,
              epochs=15, batch_size=32, callbacks=None, fit_kwargs=None)

    test_y = list(map(lambda e: list(e), zip(*test_y)))
    reports = model.evaluate((test_x, test_img), test_y, batch_size=32)
